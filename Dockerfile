...
ENV POSTFIXADMIN_BASE_URL http://downloads.sourceforge.net/project/postfixadmin/postfixadmin
...
# 3. Extract the downloaded archive, move it to the APPLICATION_PATH and remove
#    the archive
...
RUN apt-get update && \
    ...
    wget "$POSTFIXADMIN_BASE_URL/postfixadmin-$POSTFIXADMIN_VERSION/postfixadmin-$POSTFIXADMIN_VERSION.tar.gz" && \
